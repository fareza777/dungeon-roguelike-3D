...
const KILLER_NAMES := {
	...
	"salt_lich": "a Salt Lich", "bilge_fury": "a Bilge Fury", "siren_thrall": "a Siren Thrall", "pale_lantern": "a Pale Lantern",
...
func _omen_deal(idx: int) -> void:
	var osize := 99 if Stats.nemesis != "" else 98
	if idx >= osize:
		omen_refusals += 1
		if omen_refusals >= 2:
