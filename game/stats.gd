...
func earn_souls(n: int) -> void:
	...
	if dead_weight and n > 0:
		pay = maxi(0, n - 1)
	souls += int(ceilf(pay * (1.0 + soul_gain_pct) * (1.0 + 0.04 * float(meta.get("salvor", 0))) * (1.0 + 0.02 * float(meta.get("ferry", 0))) * (1.0 + 0.04 * float(meta.get("keelwright", 0)))))
	souls_run += int(ceilf(pay * (1.0 + soul_gain_pct) * (1.0 + 0.04 * float(meta.get("salvor", 0))) * (1.0 + 0.02 * float(meta.get("ferry", 0))) * (1.0 + 0.04 * float(meta.get("keelwright", 0)))))
	if souls >= 60:
		ach["deeppurse"] = true
